...
class Send_Email_To_Admin(Form):
    ...
    # 相当于<input type="textarea" placeholder="提示">
    #       <input type="submit">
...
class Confession(Form):
    # 相当于<input type="textarea" placeholder="提示" style="color:red">
    #       <input type="submit">
    # render_kw参数是一个字典，里面填写这个input表情的一些属性（提示语，CSS样式等），
    # 列如：render_kw={'placeholder':u"告白内容",'style':"color:red;text-align:right"}
    confession = TextAreaField(u'公共告白栏',validators=[Required()],render_kw={'placeholder':u"告白内容",'style':"color:red;"})
    submit = SubmitField(u'说出我的情')
