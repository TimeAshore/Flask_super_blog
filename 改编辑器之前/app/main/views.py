#encoding: utf-8
from flask import render_template, session, redirect, url_for, current_app,flash,abort,request,make_response,current_app
from .. import db
from ..models import User,Role,Post,Permission,Comment,Confess
from ..email import send_email
from . import main
from .forms import NameForm,EditProfileForm,EditProfileAdminForm,PostForm,CommentForm,Send_Email_To_Admin,Confession,Translate,Translate_answer
import sys
...
def make():
    ...
    if current_user.can(Permission.WRITE_ARTICLES) and form.validate_on_submit():
        ...
        flash(u"新文章发布成功")
        return redirect(url_for('.index'))
    return render_template('write_an_article.html',form=form)
...
def confession():
    ...
    # 添加分页
    page = request.args.get('page', 1, type=int)
    pagination = Confess.query.order_by(Confess.time.desc()).paginate(page,
